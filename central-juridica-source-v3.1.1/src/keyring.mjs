...
export function parseKeyring({ value, legacyValue, envName, legacyEnvName, production = false, devSeed, defaultKeyId = 'legacy-v1' }) {
  if (value) {
    let parsed;
    try { parsed = JSON.parse(String(value)); } catch { throw new Error(`${envName} deve ser JSON válido.`); }
    ...
    for (const [keyId, encoded] of entries) {
      if (!/^[A-Za-z0-9._:-]{1,80}$/.test(keyId)) throw new Error(`${envName}.keys contém keyId inválido.`);
      keys.set(keyId, decodeKey(encoded, `${envName}.keys.${keyId}`));
    }
  ...
  if (legacyValue) {
    return makeSingleKeyring(decodeKey(legacyValue, legacyEnvName), { keyId: defaultKeyId, source: 'legacy-env' });
  }
...
export function activeKey(keyring) {
  if (!keyring?.keys?.has(keyring.activeKeyId)) throw new Error('Keyring sem chave ativa.');
  return keyring.keys.get(keyring.activeKeyId);
}
...
export function publicKeyringStatus(keyring) {
  return { activeKeyId: keyring.activeKeyId, legacyKeyId: keyring.legacyKeyId, knownKeyIds: [...keyring.keys.keys()].sort(), keyCount: keyring.keys.size, source: keyring.source };
}
