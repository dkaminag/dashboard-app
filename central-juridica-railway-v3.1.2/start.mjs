...
async function shutdown(signal) { console.log(JSON.stringify({ level: 'info', event: 'server-shutdown', signal })); server.close(error => process.exit(error ? 1 : 0)); setTimeout(() => process.exit(1), 10000).unref(); }
